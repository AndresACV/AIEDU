...
def load_env_file():
    ...
    if os.path.exists(env_path):
        try:
            ...
            # Simple but robust parsing: split on first = per line, handle multiline values differently
            lines = content.split('\n')
            current_var = None
            current_value = []
            ...
            for line in lines:
                ...
                # Check if this starts a new variable (has = sign)
                if '=' in line_stripped:
                    # Save previous variable if exists
                    if current_var:
                        final_value = '\n'.join(current_value).strip()
                        # Remove outer quotes if present
                        if (final_value.startswith('"') and final_value.endswith('"')) or \
                           (final_value.startswith("'") and final_value.endswith("'")):
                            final_value = final_value[1:-1]
                        os.environ[current_var] = final_value
                        print(f"✅ Loaded {current_var} from .env")
                    ...
                    # This is a continuation line (for multiline values)
                    if current_var:
            ...
            if 'GOOGLE_APPLICATION_CREDENTIALS' in os.environ:
                ...
                if creds.strip().startswith('{') and creds.strip().endswith('}'):
                    print("🔑 Credentials appear to be in JSON format")
                    # Try to parse to validate
                    try:
                        json.loads(creds)
                        print("🔑 JSON credentials validated successfully")
                    except json.JSONDecodeError as je:
                        print(f"⚠️ JSON credentials validation failed: {je}")
                else:
                    print("🔑 Credentials appear to be a file path")
                    print(f"🔍 Credential value preview: {creds[:50]}...")
...
from rag_pipeline import get_rag_pipeline, ConversationMemory
...
# Configure flexible CORS for Next.js frontend (wide port range for auto-port)
# Support any localhost port that Next.js might use
allowed_origins = []
for port in range(3000, 3020):  # Extended range for any port Next.js picks
    allowed_origins.extend([f'http://localhost:{port}', f'http://127.0.0.1:{port}'])

# Add common network addresses
allowed_origins.extend(['http://192.168.1.7:3004', 'http://192.168.1.7:3000'])
...
# Import and initialize hybrid speech functionality
try:
    from hybrid_speech import get_hybrid_speech_manager
    hybrid_speech = get_hybrid_speech_manager(AUDIO_FOLDER)
    print(f"🎤 Hybrid Speech Manager initialized - Available: {hybrid_speech.is_available()}")
    print(f"🎯 Current Speech Mode: {hybrid_speech.get_current_mode()}")
except Exception as e:
    print(f"❌ Failed to initialize Hybrid Speech Manager: {e}")
    hybrid_speech = None
...
def get_model_path(language='en'):
    ...
    # Get absolute path to models directory (relative to this Flask app)
...
@app.route('/voices')
def voices():
    """Get available voices for TTS with hybrid speech support."""
    try:
        language = request.args.get('language', 'en-US')
        
        if hybrid_speech and hybrid_speech.is_available():
            # Get voices from hybrid speech manager
            voices = hybrid_speech.get_available_voices(language)
            return jsonify({
                'success': True, 
                'voices': voices,
                'hybrid_available': True,
                'current_mode': hybrid_speech.get_current_mode()
            })
        else:
            # Fallback to original voice detection
...
@app.route('/speech-modes')
def speech_modes():
    """Get available speech processing modes."""
    try:
        modes = [
            {
                'id': 'auto',
                'name': 'Auto (Intelligent)',
                'description': 'Automatically selects best provider',
                'icon': '🤖'
            },
            {
                'id': 'privacy',
                'name': 'Privacy First',
                'description': 'Uses local processing only',
                'icon': '🔒'
            },
            {
                'id': 'performance',
                'name': 'Performance',
                'description': 'Prioritizes cloud quality and speed',
                'icon': '⚡'
            },
            {
                'id': 'cost',
                'name': 'Cost Conscious',
                'description': 'Minimizes cloud API usage',
                'icon': '💰'
            }
        ]
        ...
        print(f"Error getting speech modes: {e}")
        return jsonify({'success': False, 'error': str(e)})
...
def set_speech_mode():
    ...
    try:
        ...
        if hybrid_speech and hybrid_speech.is_available():
            success = hybrid_speech.set_speech_mode(mode)
            if success:
                return jsonify({
                    'success': True,
                    'mode': hybrid_speech.get_current_mode(),
                    'message': f'Speech mode set to {mode}'
                })
            else:
                return jsonify({
                    'success': False,
                    'error': f'Failed to set speech mode to {mode}'
                })
        else:
            return jsonify({
                'success': False,
                'error': 'Hybrid speech functionality not available'
            })
    except Exception as e:
        print(f"Error setting speech mode: {e}")
        return jsonify({'success': False, 'error': str(e)})

@app.route('/speech-stats')
def speech_stats():
    """Get speech processing performance statistics."""
    try:
        if hybrid_speech and hybrid_speech.is_available():
            stats = hybrid_speech.get_performance_stats()
            return jsonify({
                'success': True,
                'stats': stats
            })
...
@app.route('/force-provider', methods=['POST'])
def force_provider():
    """Force usage of specific providers (local, cloud, or auto)."""
    try:
        data = request.json
        provider_type = data.get('provider', 'local')  # auto, local, cloud
        
        print(f"🔄 Provider selection: {provider_type}")
        
        # Store provider preference (for now we'll just acknowledge the request)
        # In full hybrid implementation, this would actually switch providers
        
        # Map provider types for display
        provider_messages = {
            'local': 'Using local providers (Vosk + espeak + Ollama)',
            'cloud': 'Using cloud providers (Google Cloud + Gemini)', 
            'auto': 'Using automatic provider selection'
        }
        
        message = provider_messages.get(provider_type, f'Provider set to {provider_type}')
        
        if hybrid_speech and hasattr(hybrid_speech, 'set_speech_mode'):
            # Try to use hybrid speech if available
            mode_mapping = {
                'auto': 'auto',
                'local': 'privacy',  # Privacy mode forces local
                'cloud': 'performance'  # Performance mode forces cloud
            }
            
            if provider_type in mode_mapping:
                try:
                    mode = mode_mapping[provider_type]
                    success = hybrid_speech.set_speech_mode(mode)
                    
                    if success:
                        return jsonify({
                            'success': True,
                            'provider_forced': provider_type,
                            'speech_mode': hybrid_speech.get_current_mode(),
                            'message': f'Provider switched to {provider_type}'
                        })
                except Exception as e:
                    print(f"Hybrid speech mode change failed: {e}")
        
        # Fallback - just acknowledge the request
        return jsonify({
            'success': True,
            'provider_forced': provider_type,
            'speech_mode': provider_type,
            'message': message
        })
...
def current_providers():
    ...
    try:
        ...
        # Check Vosk STT availability
        try:
            import vosk
            import os
            model_path_en = get_model_path('en')
            model_path_es = get_model_path('es')
            if model_path_en or model_path_es:
                providers['stt_provider'] = 'Vosk (Local)'
            else:
                providers['stt_provider'] = 'Vosk (Models Missing)'
        except ImportError:
            providers['stt_provider'] = 'Vosk (Not Installed)'
        except Exception as e:
            providers['stt_provider'] = f'Vosk (Error: {str(e)[:20]})'
        ...
        try:
            ...
            if platform.system() == 'Linux':
                ...
                result = subprocess.run(['which', 'espeak'], capture_output=True)
                if result.returncode == 0:
                    providers['tts_provider'] = 'espeak (Local)'
                else:
                    providers['tts_provider'] = 'espeak (Not Installed)'
            else:
                # Check for pyttsx3
                import pyttsx3
                test_engine = pyttsx3.init()
                if test_engine:
                    providers['tts_provider'] = 'pyttsx3 (Local)'
                else:
                    providers['tts_provider'] = 'pyttsx3 (Failed)'
        except Exception as e:
            providers['tts_provider'] = f'TTS (Error: {str(e)[:20]})'
        
        # Check LLM provider
        try:
            # Check for Gemini API
            if 'GEMINI_API_KEY' in os.environ and os.environ['GEMINI_API_KEY'].strip():
                providers['llm_provider'] = 'Gemini 2.5 Flash (Cloud)'
            else:
                # Check for Ollama
                import requests
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if models:
                            providers['llm_provider'] = f'Ollama (Local: {len(models)} models)'
                        else:
                            providers['llm_provider'] = 'Ollama (No Models)'
                    else:
                        providers['llm_provider'] = 'Ollama (Not Responding)'
                except:
                    providers['llm_provider'] = 'Ollama (Not Available)'
        except Exception as e:
            providers['llm_provider'] = f'LLM (Error: {str(e)[:20]})'
        
        # Check if hybrid speech is available
        if hybrid_speech and hasattr(hybrid_speech, 'is_available'):
            providers['hybrid_available'] = hybrid_speech.is_available()
        
        return jsonify({
            'success': True,
            'providers': providers,
            'current_mode': hybrid_speech.get_current_mode() if hybrid_speech and hasattr(hybrid_speech, 'get_current_mode') else 'local_only'
        })
        
    except Exception as e:
        print(f"Error getting current providers: {e}")
        return jsonify({'success': False, 'error': str(e)})

@app.route('/synthesize', methods=['POST'])
def synthesize():
    """Convert text to speech with hybrid speech provider support."""
    try:
        data = request.json
        text = data.get('text', '')
        ...
        print(f"🎤 TTS Request - Text: '{text[:50]}...', Voice: '{voice_id}', Language: '{language}'")
        
        if not text:
            return jsonify({'success': False, 'error': 'No text provided'})
        
        # Try hybrid speech first
        if hybrid_speech and hybrid_speech.is_available():
            try:
                print(f"🚀 Using Hybrid Speech Manager - Mode: {hybrid_speech.get_current_mode()}")
                result = hybrid_speech.synthesize_speech(text, language, voice_id)
                
                if result.get('success'):
                    print(f"✅ Hybrid TTS Success - Provider: {result.get('provider')}, Duration: {result.get('duration', 0):.2f}s")
                    return jsonify({
                        'success': True,
                        'audio_url': result['audio_url'],
                        'file_size': result.get('file_size', 0),
                        'provider': result.get('provider', 'Hybrid'),
                        'voice_used': result.get('voice_used', voice_id),
                        'language': result.get('language', language),
                        'duration': result.get('duration', 0),
                        'cached': result.get('cached', False),
                        'message': f"Audio generated using {result.get('provider', 'Hybrid')} in {result.get('duration', 0):.2f}s"
                    })
                else:
                    print(f"⚠️ Hybrid TTS failed: {result.get('error', 'Unknown error')} - Falling back to legacy")
            except Exception as hybrid_error:
                print(f"❌ Hybrid TTS error: {hybrid_error} - Falling back to legacy")
        
        # Fallback to original TTS implementation
        print("🔄 Using legacy TTS implementation")
...
def speech_to_rag():
    ...
    try:
        ...
        # Process speech to text using hybrid speech manager
        logger.info(f"Starting transcription for {audio_filename}")
        
        # Try hybrid speech first
        if hybrid_speech and hybrid_speech.is_available():
            try:
                logger.info(f"🚀 Using Hybrid Speech Manager - Mode: {hybrid_speech.get_current_mode()}")
                transcription_result = hybrid_speech.transcribe_audio(audio_path, language)
                
                if transcription_result.get('success'):
                    logger.info(f"✅ Hybrid STT Success - Provider: {transcription_result.get('provider')}, Confidence: {transcription_result.get('confidence', 0):.2f}")
                else:
                    logger.warning(f"⚠️ Hybrid STT failed: {transcription_result.get('error', 'Unknown error')} - Falling back to legacy")
                    transcription_result = transcribe_audio_file(audio_path, language)
            except Exception as hybrid_error:
                logger.error(f"❌ Hybrid STT error: {hybrid_error} - Falling back to legacy")
                transcription_result = transcribe_audio_file(audio_path, language)
        else:
            logger.info("🔄 Using legacy STT implementation")
            transcription_result = transcribe_audio_file(audio_path, language)
        ...
        text = transcription_result.get('transcript', transcription_result.get('text', ''))
...
def generate_self_signed_cert():
    ...
    # Check if certificate already exists (fixed path)
...
if __name__ == "__main__":
    ...
    RELOAD_MODE = os.environ.get('AIEDU_RELOAD', 'false').lower() == 'true'
    ...
    try:
        ...
        # Start with optimized settings (configurable reload)
        app.run(
            ...
            use_reloader=RELOAD_MODE,            # Configurable auto-reload
